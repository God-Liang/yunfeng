<template lang="pug">
  .lg-container
    .lg-container-white
      .filter-container
        el-radio-group.type(v-model="filters.scope" @change="handleScope")
        ...
        .filter-item
          ...
          el-button.filter-item(type="primary" icon="el-icon-plus" @click="onCreate") 新增
      lgSelectGroup(v-if="filters.scope === '0'" :selectData="followUpPlan.statusArr" :value="filters.status" title="状态：" @click="clickStatus")
        ...
        .lg_formList
          .scale_l
            .scale_l_item(v-for="(nodeVal, node_i) in nodeList[nodeActive].formSolutionNodeItemList" :key="node_i")
              .scale_l_item_title {{nodeVal.templateName}}({{nodeVal.scopeShow}})
          ...
          .mission_r
            .mission_r_item(v-for="(nodeVal, node_i) in nodeList[nodeActive].visitSolutionNodeItemList" :key="node_i")
              .mission_r_item_title {{nodeVal.templateName}}({{nodeVal.scopeShow}})
...
import { list, create, createOrUpdate } from '@/api/common'
import { shareSolution } from '@/api/share'
...
export default {
  name: 'ComponentName',
  components: { lgTable, lgDialog, lgSelectGroup, selectForm, nodeInfo, lgSolt },
  data() {
    return {
      followUpPlan,
  ...
  methods: {
    ...
    onSubmit() {
      ...
      dataObj.solution.scope = 0
      dataObj.solution.scopeShow = '个人'
