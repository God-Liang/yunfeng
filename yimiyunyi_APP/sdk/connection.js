...
import im from '@/utils/WebIMConfig';
let WebIM = {};
...
function _login(options, conn) {
	...
	conn.context.accessToken = accessToken;
	conn.context.accessTokenExpires = expires_in;
...
connection.prototype.open = function(options) {
	...
	if (options.accessToken) {
		...
		_utils.ajax({
			url: apiUrl + "/" + this.orgName + "/" + this.appName + "/token",
			data: loginfo,
...
connection.prototype.sendCommand = function(dom, id) {
	...
	if (this.isOpened()) {
		...
		// 缓存未发送成功消息
		uni.setStorageSync('fail_msgId', id)
		//尝试使用token重新登陆
		if (!me.isOpened() && uni.getStorageSync("myUsername") && uni.getStorageSync("myToken")) {
			helper.loading('连接超时,自动登录中...', true);
			me.open({
				apiUrl: im.apiURL,
			...
			helper.toast('none', '连接超时,请重新登录', true);
			setTimeout(() => {
...
connection.prototype.queryRoomInfo = function(options) {
	...
	var completeFn = function(result) {
		...
		if (features) {
			settings = features[1].getAttribute("var") + "|" + features[3].getAttribute("var") + "|" + features[4].getAttribute(
				"var");
		}
		switch (settings) {
...
// 修改密码
connection.prototype.changePassword = function(opt) {
	var options = {
		url: this.apiUrl + "/" + this.orgName + "/" + this.appName + "/users/" + opt.uid + "/password",
		type: "GET",
		dataType: "json",
		data: opt.data,
		headers: {
...
connection.prototype.listGroups = function(opt) {
	var t = {};
	if (t.limit = opt.limit, t.cursor = opt.cursor, t.cursor || delete t.cursor, isNaN(opt.limit)) throw 'The parameter "limit" should be a number';
	let options = {
...
// 通过Rest根据groupid修�����群组详情
connection.prototype.setGroupInfo = function(opt) {
	var options = {
		url: this.apiUrl + "/" + this.orgName + "/" + this.appName + "/chatgroups/" + opt.groupId,
		type: "PUT",
		dataType: "json",
...
connection.prototype.agreeJoinGroup = function(opt) {
	var options = {
		url: this.apiUrl + "/" + this.orgName + "/" + this.appName + "/chatgroups/" + opt.groupId + "/apply_verify",
		type: "POST",
		dataType: "json",
		data: JSON.stringify({
...
connection.prototype.removeAdmin = function(opt) {
	var groupId = opt.groupId;
	var options = {
		url: this.apiUrl + "/" + this.orgName + "/" + this.appName + "/chatgroups/" + groupId + "/admin/" + opt.username,
		type: "DELETE",
		dataType: "json",
		headers: {
			Authorization: "Bearer " + this.context.accessToken,
