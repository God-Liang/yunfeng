...
<template>
	<view class="mescroll-empty" :class="{ 'empty-fixed': option.fixed }" :style="{ 'z-index': option.zIndex, top: option.top }">
		...
		<view v-if="describe" class="empty-describe">{{ describe }}</view>
...
export default {
	...
	computed: {
		...
		// 文本描述
		describe() {
			return this.option.describe == null ? GlobalOption.up.empty.describe : this.option.describe; // 此处不使用短路求值, 用于支持传空串不显示文本提示
		}
	},
	methods: {
		// 点击按钮
