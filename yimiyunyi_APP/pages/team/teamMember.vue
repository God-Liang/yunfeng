<template>
	<view class="lg-container">
		<uni-nav-bar left-icon="back" :title="'团队成员（' + total + '）'" @clickLeft="onLeftTap"></uni-nav-bar>
		...
		<view class="lg_list">
			<mescroll-uni ref="mescrollRef" @init="mescrollInit" :fixed="false" @down="downCallback" @up="upCallback" :down="downOption" :up="upOption">
				<view class="lg_item" v-for="(item, index) in dataList" :key="item.id" @tap="onJump('/pages/team/teamMemberInfo?userId='+item.userId)">
					<view class="lg_item_img">
						<image class="lg_item_img_img" v-if="item.avatar" :src="item.avatar" mode="aspectFill"></image>
						<image class="lg_item_img_img" v-else src="@/static/team/avatar_doctor.png" alt="aspectFit"></image>
					</view>
					<view class="lg_item_box">
						...
						<view class="lg_item_box_info">
							<text class="lg_item_box_info_professionCall">{{ item.jobType || '暂无职称' }}</text>
							<text class="lg_item_box_info_department ml20">{{ item.department || '暂无科室' }}</text>
						</view>
					</view>
				</view>
			</mescroll-uni>
