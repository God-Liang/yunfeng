	...
	<view class="lg-container">
		...
		<view class="patient_list">
			<mescroll-uni ref="mescrollRef" @init="mescrollInit" :fixed="false" @down="downCallback" @up="upCallback" :down="downOption" :up="upOption">
				<view class="patient_item" :class="selectList.indexOf(item.patientId) >= 0 ? 'patient_item_acitve' : ''" v-for="(item, index) in dataList" :key="item.id" @tap="handleChange(item)">
		...
		<view class="operationBox" @tap="onNext">下一步</view>
	</view>
</template>
...
export default {
	...
	onShow() {
		this.selectList = this.planObj.ids;
		this.selectObjList = this.planObj.objIds;
	},
	methods: {
		onLeftTap() {
		...
		// 选择患者
		handleChange(item) {
			if (this.selectList.length > 0 && this.selectList[0] === item.patientId) {
				this.selectList = []
				this.selectObjList = []
			} else {
				this.selectList = [item.patientId]
				this.selectObjList = [{
			...
			// if (this.selectList.indexOf(item.patientId) >= 0) {
			// 	this.selectList.splice(this.selectList.indexOf(item.patientId), 1);
			// 	this.selectObjList.splice(this.selectObjList.indexOf(item.patientId), 1);
			// } else {
			// 	this.selectList.push(item.patientId);
			// 	this.selectObjList.push({
			// 		name: item.patientName,
			// 		sex: item.sex,
			// 		avatar: item.avatar
			// 	})
		...
		onNext() {
			...
			this.planObj.ids = this.selectList;
			this.planObj.objIds = this.selectObjList;
