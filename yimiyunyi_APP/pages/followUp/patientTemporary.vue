<template>
	<view class="lg-container">
		<uni-nav-bar left-icon="back" title="临时随访病人" @clickLeft="onLeftTap"></uni-nav-bar>
		<view class="patient_list">
			<mescroll-uni ref="mescrollRef" @init="mescrollInit" :fixed="false" @down="downCallback" @up="upCallback" :down="downOption" :up="upOption">
				<view class="patient_item" v-for="(item, index) in dataList" :key="index">
					<view class="patient_item_box" @tap="onJump('/pages/followUp/patientTemporaryDetails?patientId='+item.patientId+'&patientName='+item.patientName)">
						<view class="patient_item_box_info">
							...
							<view class="patient_item_box_info_more">
								<view class="newBtn" @tap.stop="onTemporary(item)">临时随访</view>
								<view class="newBtn ml20" @tap.stop="onPlan(item)">新建计划</view>
							</view>
						</view>
