      ...
      <div class="follower">
        <el-form ref="formLeftTop2" :model="formLeftTop2" :label-width="labelWidth" :inline="true">
          ...
          <el-form-item label="是否完成">
            <el-select v-model="formLeftTop2.isFinish" placeholder="请选择" style="width: 200px;">
...
export default {
  ...
  mounted() {
    ...
    getFollowupDetail({ id: id }).then(res => {
      ...
      this.formLeftTop2.execUserName = this.nick
    ...
    const currentTime = new Date().getFullYear() + '-' + ((new Date().getMonth() + 1) < 10 ? '0' + (new Date().getMonth() + 1) : (new Date().getMonth() + 1)) + '-' + (new Date().getDate() < 10 ? '0' + new Date().getDate() : new Date().getDate())
    this.formLeftTop2.execTime = currentTime
    this.getHintList()
  },
  data() {
    return {
      ...
      this.$refs[name].validate((valid) => {
        if (valid && renderFormValid) {
          ...
          submitFollowup(query).then(res => {
            if (res.data.code === 200) {
              this.$message({ message: '随访提交成功!', type: 'success' })
              // this.$store.commit('SET_SECONDARYJUMP', '')
              this.$root.closeTag()
            }
          })
        } else {
