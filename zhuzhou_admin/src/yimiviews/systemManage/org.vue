      ...
      <div class="flexWrap">
        <div class="leftWrap">
          ...
          <complexTable
            ...
            :list = 'list'
            :totalList = 'total'
            :theadList = "orgConst.theadList1"
            :operationButtons = 'orgConst.operationButtons1'
            :statusArr = 'orgConst.statusArr'
...
<script>
    ...
    import { orgConst } from '@/yimiviews/systemManage/columnsConst'
    import { getList, save, remove, update } from '@/api/systemManage/org'
    import { updateStatus } from '@/api/systemManage/user'
    ...
    export default {
      ...
      methods: {
        ...
        operationEvent1_2(row) {
          this.$confirm(row.status === 0 ? '审核是否通过?' : '是否禁用?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            updateStatus({ id: row.adminId, status: row.status === 0 ? 1 : 0 }).then((res) => {
              this.$message({
                type: 'success',
                message: row.status === 0 ? '审核通过' : '禁用成功'
              })
              this.handleFilter()
            })
          }).catch(() => {
            this.$message({
              type: 'info',
              message: row.status === 0 ? '已取消审核' : '已取消禁用'
            })
          })
        }
      }
