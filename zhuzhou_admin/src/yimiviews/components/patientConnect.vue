  ...
  <div class="confirmWrap">
    <div slot="footer" class="dialog-footer clearfix">
      ...
      <div style="float: right;">
        ...
        <el-button type="primary" class="buttonRadius sumbitColor" @click="submitForm">完成</el-button>
      </div>
    </div>
    <div class="registrationBox">
      <div style="width: 970px;padding: 15px 22px;margin: 0 auto;">
        ...
        <p style="font-size:10.5pt; line-height:150%; margin:0pt; orphans:0; widows:0">
          ...
          >{{handoverForm.registerDept}}</span>
          <span style="font-family:宋体; font-size:13px;">床号</span>
          <span
            style="font-family:宋体; font-size:12px; border-bottom:1px solid #333;text-align:center; min-width:50px;display:inline-block;line-height: 12px;"
          >{{handoverForm.wardBedNo}}</span>
...
<script>
import { read, save } from '@/api/optReady/handover'
export default {
  name: 'patientConnect',
  data() {
    return {
    ...
    if (this.$route.params.id) {
      read(parseInt(this.$route.params.id)).then(res => {
        if (res.data.code === 200) {
          ...
          this.visitDepartment = res.data.registerDept
        }
      })
    }
  },
  methods: {
    ...
    submitForm() {
      const appointmentStatusArray = this.handoverForm.appointmentStatusArray.split(',')
      const arr = []
      if (appointmentStatusArray.indexOf('10') < 0) {
        arr.push('麻醉评估')
      }
      if (appointmentStatusArray.indexOf('11') < 0) {
        arr.push('中心核对')
      }
      const promptMsg = arr.length === 0 ? '是否确认完成患者交接？' : arr.join('、') + '暂未完成，是否确认完成患者交接？'
      this.$confirm(promptMsg, '提示', {
        ...
        const data = {
          ...
          formInfoId: 1,
          jsonResult: JSON.stringify(this.handoverForm),
          htmlContent: document.querySelector('.registrationBox').innerHTML
        }
        save(data).then(res => {
          if (res.data.code === 200) {
            this.$message({
